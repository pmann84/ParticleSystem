...
{	
	...
	std::shared_ptr<ps::idomain> source_domain = std::make_shared<ps::disk_domain>(ps::vector3d(0.0f, 0.0f, 0.0f), 0.1, 0.25, M_PI / 2, M_PI / 2);

	std::shared_ptr<ps::idomain> velocity_domain = std::make_shared<ps::disk_domain>(ps::vector3d(0.0f, 5.0f, 0.0f), 0.75, 1.5, M_PI/2, M_PI/2);
	...
	std::shared_ptr<ps::idomain> sink_domain = std::make_shared<ps::disk_domain>(ps::vector3d(0.0f, -1.0f, 0.0f), 0.0, 5.0, M_PI / 2, M_PI / 2);
	std::shared_ptr<ps::idomain> bounce_domain = std::make_shared<ps::disk_domain>(ps::vector3d(0.0f, -0.5f, 0.0f), 0.0, 2.0, M_PI / 2, M_PI / 2);
	...
	p_group.source(source_domain)
		   .velocity(velocity_domain, 2.5, true, 4)
		   .sink(sink_domain)
		   .stick(bounce_domain)
